...
export class HobbyComponent implements OnInit {
  ...
  list(){
    this.api.post("hobby/list", {}).subscribe((mydata:any)=>{
  ...
  edit(id:string):void{
    this.id = id;
    this.api.post("hobby/get", {id:this.id}).subscribe((mydata:any)=>{
        this.id = mydata.data._id;
        this.title = mydata.data.title;
        this.initialize();
      });
  ...
  delete(id:string):void{
    if(confirm("Sure to delete?")){
      let data = {id:id};
      this.api.post("hobby/delete", {id}).subscribe((mydata)=>{
        this.list();
      });
    }
  }
  ...
  submit(formdata:any)
  {
    ...
    this.api.post("hobby/save", formdata).subscribe((mydata)=>{
